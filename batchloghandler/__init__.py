...
def record(*args):
    ...
    [h.start_recording() for h in logger.handlers if isinstance(h, BatchLogHandlerBase)]

def stop(*args):
    logger = logging.getLogger(*args)
    [h.stop_recording() for h in logger.handlers if isinstance(h, BatchLogHandlerBase)]


def BatchLogHandler(blh_backend, blh_bypass_level, blh_trigger_level, **kwargs):
